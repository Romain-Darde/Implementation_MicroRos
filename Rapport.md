 ...
 Afin de ne pas compromettre le code de production existant pour la phase 2, j’ai préféré créer un nouveau dépôt Git (Implementation_MicroRos) pour cette dernière phase.
 ...
 Tableau de Bord d’implémentation et erreur traversé sur le chemin :
 ...
 Il m’a fallu installé aussi cette nouvelle librairie :

 ```bash
 pip install catkin_pkg empy lark
 ```

 Il m’a aussi fallu désactiver temporairement le ROS2 de mon ordinateur car j’avais un conflit d'environnement et cross-compilation:
 ...
 L'origine du problème provenait de l'environnement de développement : l'outil de construction de ROS 2 (colcon), exécuté par Zephyr en arrière-plan, scannait les variables d'environnement de l'ordinateur hôte (Ubuntu). Il incluait alors accidentellement les bibliothèques natives du PC (définies dans ~/.bashrc via /opt/ros/humble), corrompant ainsi la compilation croisée (cross-compilation) destinée au processeur ARM de la STM32.
 La résolution de ce conflit a nécessité une isolation stricte de l'environnement Zephyr. Les variables systèmes liées à ROS (AMENT_PREFIX_PATH, PYTHONPATH, LD_LIBRARY_PATH) ont dû être effacées du terminal avant la compilation pour forcer l'outil à générer des binaires exclusivement dédiés à la cible embarquée.

 Alors j’ai fais ces commandes:
 ...
 Cela n’a pas suffit alors temporairement, j’ai commenté dans le fichier bash de mon terminal:
 ...
 Enregistrer puis quitter. Dans un tout nouveau terminal cette fois-ci:
 ...
 Après avoir passé cette erreur un autre gros problème a été la RAM de la carte Nucleo, voici les erreurs que j'obtenais dans le terminal:
 ...
 Car la carte Nucleo STM32F103RB ne dispose que de 20 Ko de RAM. Avec ses paramètres par défaut, Micro-ROS (et particulièrement son middleware de communication XRCE-DDS) provisionne d'importants buffers de fiabilité et d'historique, exigeant près de 40 Ko de RAM, ce qui provoque un dépassement critique ("overflow") de la section .bss.

 Puisque le robot n'a besoin que d'un seul Nœud et d'un seul Publisher (pour envoyer l'angle), nous allons désactiver tout le reste pour faire rentrer Micro-ROS dans les 20 Ko de la STM32.
 ...
 J’ai dû avoir recours à un bridage structurel de Micro-ROS (prj.conf) : j’ai strictement limitée au besoin du Proof of Concept : 1 seul Nœud, 1 seul Publisher, et 0 Subscriber/Service. De plus, les tampons d'historique réseau (RMW_HISTORIC et XRCE_DDS_HISTORIC) ont été abaissés à 1 seul message, supprimant toute allocation mémoire superflue.
 ...
 Une fois la compilation réussi dans un autre terminal :
